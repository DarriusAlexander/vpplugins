...
function wpdm_getlink()
{
    ...
    if (isset($_POST['force']) && $_POST['force'] != '') {
        ...
        if ($vr[0] == 'unlocked') {
            ...
            \WPDM\Session::set('__wpdm_unlocked_'.$file['ID'], 1);
...
function wpdm_embed_category($params = array('id' => '', 'operator' => 'IN' , 'items_per_page' => 10, 'title' => false, 'desc' => false, 'order_field' => 'create_date', 'order' => 'desc', 'paging' => false, 'toolbar' => 1, 'template' => '','cols'=>3, 'colspad'=>2, 'colsphone' => 1, 'morelink' => 1))
{
    ...
    $order_field = (!isset($_GET['orderby']) && isset($order_field))?$order_field:
...
function wpdm_post_thumb($size='', $echo = true, $extra = null){
    ...
    $crop = isset($extra['crop'])?$extra['crop']:false;
    $alt = $post->post_title;
    if(is_array($size))
    {
 ...
 * @return string
 */
function wpdm_sanitize_var($value){
    if(is_array($value))
