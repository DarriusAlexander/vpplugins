...
//        ->Fetch('file-list.php');
        ...
        if (count($file['files']) > 0 || count($dfiles) > 0) {
            ...
            if(!Package::userCanDownload($file['ID']) || Package::isLocked($file['ID'])) { $olock = 1; $noaccess = 1; }
            ...
            if(isset($package['terms_lock']) && $package['terms_lock'] != 0 && (!function_exists('wpdmpp_effective_price') || wpdmpp_effective_price($package['ID']) ==0)) {
                $cattr = "data-termlock=1";
                $data = "<div id='term-panel-{$package['ID']}' class='panel panel-default terms-panel' style='display:none;;margin: 0;position: absolute;width: 100%;height: 100%;z-index: 999;background: rgba(255,255,255,0.9);'><div class='panel-heading'>{$package['terms_title']}</div><div class='panel-body' style='max-height: 200px;overflow: auto'>{$package['terms_conditions']}</div><div style='position: absolute;width: 100%;height: 40px;bottom: 0;z-index: 99' class='panel-footer'><label><input data-pid='{$package['ID']}' class='wpdm-checkbox terms_checkbox terms_checkbox_{$package['ID']}' type='checkbox'> {$package['terms_check_label']}</label></div></div>";
            }

            $fhtml = "<audio autoplay controls preload='auto' style='width: 100%;display: none' id='audio-player-{$file['ID']}'></audio><div {$cattr} data-packageid='{$file['ID']}' id='wpdm-filelist-area-{$file['ID']}' class='wpdm-filelist-area wpdm-filelist-area-{$file['ID']}' style='position:relative'>{$data}<table id='wpdm-filelist-{$file['ID']}' class='wpdm-filelist table table-hover'><thead><tr><th>".__( "File" , "download-manager" )."</th>{$pwdcol}{$dlcol}</tr></thead><tbody>";
        ...
        if (count($file['files']) > 0 || count($dfiles) > 0) {
            ...
            if (is_array($dfiles)) {
                ...
                foreach ($dfiles as $ind => $sfile) {
                    ...
                    $classes = array('1' => 'col-md-12', '2' => 'col-md-6', '3' => 'col-md-4', '4' => 'col-md-3', '6' => 'col-md-2');
                    $class = isset($classes[$cols])?$classes[$cols]:'col-md-4';
                    $fhtml .= "<div class='{$class}'><div class='wpdm-file-block'>";
                    if (!isset($fileinfo[$sfile]) || !@is_array($fileinfo[$sfile])) $fileinfo[$sfile] = array();
                    if(!isset($fileinfo[$sfile]['password'])) $fileinfo[$sfile]['password'] = "";
                    ...
                    if($thumb)
                        $fhtml .= "<div class='img-area'><img class='file-thumb' src='{$thumb}' alt='{$ttl}' /></div>";
                    else
                        $fhtml .= "<div class='img-area'><img class='file-ico file-ico-{$w}x{$h}' src='".$fticon."' alt='{$ttl}' /></div>";

                    $fhtml .= "<div class='file-info'>".wpdm_file_size($sfile)."<div class='file-title ttip' title='{$ttl}'>{$cttl}</div></div>";
